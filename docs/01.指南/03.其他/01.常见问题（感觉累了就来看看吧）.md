...
## 使用 `pnpm` 安装项目依赖时报 `No matching version found for 'xxxx'`

![npm-source](~@alias/img/guide/npm-source.jpg)

答：当遇到类似上图报错时，请将您本地的 `npm` 源切换到 `npm` 官方源，暂时不要使用淘宝或者别的镜像（因为它们不是实时同步的），切换方法参考 [第二条](/pages/start/#开发环境)
