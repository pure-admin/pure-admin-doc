...
[main.ts](https://gitee.com/yiming_chang/vue-pure-admin/blob/main/src/main.ts#L29)采用`axios`在项目初始化前先请求全局配置文件，成功获取后再挂载 vue
...
谈到跨域，我们能马上反应到，在上面提到的配置文件`vite.config.ts`,有个服务配置 `proxy`，它接收一个`{ key: options }` 对象。
...
在这里我们做了个封装， `proxy` 接收的对象由方法 `createProxy` 返回。（build/proxy.ts）
...
export function createProxy(list: ProxyList = []) {
  ...
  return ret; // 返回 ret 对象
}
```

`createProxy` 的参数`VITE_PROXY`同样经过了如下处理（`loadEnv`是 vite 的一个 API，`warpperEnv` 是在`build/utils.ts`中暴露出来的方法）
...
实际开发中，我们只需要修改变量`VITE_PROXY`的参数即可
