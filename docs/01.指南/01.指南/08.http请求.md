...
来到 [build/plugins.ts](https://gitee.com/yiming_chang/pure-admin-thin/blob/main/build/plugins.ts#L7) 文件，将 `import { viteMockServe } from "vite-plugin-mock";` 注释，最后把 [这里选中](https://gitee.com/yiming_chang/pure-admin-thin/blob/main/build/plugins.ts#L51-61) 的都注释即可
...
:::tip
当然平台不仅提供了 `http.request` 方法让您发送请求，还提供脱离平台封装的请求拦截和响应拦截这种单独的 [http.post](https://gitee.com/yiming_chang/pure-admin-thin/blob/main/src/utils/http/index.ts#L176)、[http.get](https://gitee.com/yiming_chang/pure-admin-thin/blob/main/src/utils/http/index.ts#L185) 请求方式来满足一些使用场景
:::
...
① 本地开发的话，我们来到 [vite.config.ts](https://gitee.com/yiming_chang/pure-admin-thin/blob/main/vite.config.ts#L50)，参考下面代码配置本地跨域代理即可
...
④ 上面都完成后，我们在需要调用该接口的地方调用即可，参考 [src/store/modules/user.ts](https://gitee.com/yiming_chang/pure-admin-thin/blob/main/src/store/modules/user.ts#L43)
