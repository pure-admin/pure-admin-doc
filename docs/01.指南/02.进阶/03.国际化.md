...
② 使用平台提供的 [transformI18n](https://gitee.com/yiming_chang/vue-pure-admin/blob/main/src/plugins/i18n.ts#L38) 函数
...
在 `.ts` 文件中推荐使用平台提供的 [transformI18n](https://gitee.com/yiming_chang/vue-pure-admin/blob/main/src/plugins/i18n.ts#L38) 函数，因为 `useI18n` 必须在 `setup` 中使用，具体原因请看 [vue-i18n 文档](https://vue-i18n.intlify.dev/guide/advanced/composition.html#basic-usage)
...
这里的 [$t](https://gitee.com/yiming_chang/vue-pure-admin/blob/main/src/plugins/i18n.ts#L62) 是指平台提供的函数，是配合 `i18n Ally` 插件在 `vscode` 里带来智能提示，无实际意义，只对提示起作用，而非 `vue-i18n` 里的
...
const locale = storageLocal.getItem<StorageConfigs>("responsive-locale")?.locale
